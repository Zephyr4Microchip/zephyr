...
/* Register Log Module */
LOG_MODULE_REGISTER(adc_mchp_v1, CONFIG_ADC_LOG_LEVEL);
...
/* Macro to calculate the sample length in adc clock cycles */
...
/* If SUPC API support is not available, perform SUPC-related settings here */
...
/**
 * @brief ADC channel configuration for Microchip ADC driver.
 *
...
/**
 * @brief Runtime driver data for Microchip ADC driver.
 *
...
typedef struct adc_mchp_dev_data {
	...
	/**
	 * @brief Pointer to the start of the sample buffer.
	 *
...
/**
 * @brief Clock configuration structure for the ADC.
 *
...
/**
 * @brief Configuration structure for Microchip ADC driver.
 *
 * This structure holds the static configuration for the ADC hardware,
 * including HAL instance, pin configuration, clock settings, and setup function.
 */
...
/* validate the channel config parameters */
static inline int8_t adc_validate_ch_cfg_params(enum adc_gain gain, enum adc_reference reference,
						...
						uint8_t input_negative)
{
	...
	/* Validate the input_positive */
	if (input_positive < 0 || input_positive > 0x1E ||
	    (input_positive >= 0x10 && input_positive <= 0x17)) {
		ret = -EINVAL;
	}
	/* Validate input negative */
	if (input_negative < 0 || input_negative > 7) {
		ret = -EINVAL;
	}
...
/* Set ADC acquisition time in microseconds. */
...
static inline int8_t adc_set_reference(adc_registers_t *adc_reg, enum adc_reference reference)
{
	uint8_t refctrl = 0;
	int8_t ret = 0;
	...
	case ADC_REF_VDD_1_4:
		...
		break;
	default:
		printk("\nHAL Exception : Selected reference is not valid : %d\n", reference);
		ret = -EINVAL;
	}
	...
	/* It may done in the user appication, in that case don't do anything here */
#endif
...
/* Set ADC gain correction. */
static inline int8_t adc_set_gain(adc_registers_t *adc_reg, enum adc_gain gain)
{
	int8_t ret = 0;
...
/* Set the ADC positive input channel. */
static inline int8_t adc_set_input_positive(adc_registers_t *adc_reg, uint8_t input_positive)
{
	int8_t ret = 0;
	...
	default:
		/* No SUPC settings needed for other inputs */
		break;
	}
#else
	/* Use MCHP SUPC API to configure sensors and internal bandgap voltage */
#endif

	return ret;
}

/* Set the ADC negative input channel. */
static inline int8_t adc_set_input_negative(adc_registers_t *adc_reg, uint8_t input_negative,
					    bool differential)
{
	int8_t ret = 0;

	/* Validate the input_negative */
	if (input_negative < 0 || input_negative > 7) {
		ret = -EINVAL;
	}
	/* Set the ADC_INPUTCTRL Register*/
...
/* Set the number of oversampling cycles. */
static inline int8_t adc_set_oversampling(adc_registers_t *adc_reg, uint8_t oversampling)
{
	int8_t ret = 0;
...
static inline int8_t adc_set_resolution(adc_registers_t *adc_reg, uint8_t resolution,
...
static inline uint8_t adc_get_interrupt_flag(adc_registers_t *adc_reg)
{
	return adc_reg->ADC_INTFLAG;
}
...
static inline int8_t adc_set_prescalar(adc_registers_t *adc_reg, uint16_t prescaler)
{
	int8_t ret = 0;
	...
	default:
		 ...
		 */
		prescaler_val = ADC_CTRLA_PRESCALER_DIV2;
		printk("Warning: Invalid ADC prescaler value %d, using default (DIV2).\n",
		       prescaler);
		break;
	}
	if (ret == 0) {
...
{
	/* RESRDY 1 OVERRUN 2 WINMON 4*/
	adc_reg->ADC_INTFLAG = ADC_INTFLAG_Msk;
}
...
/**
 * @brief Selects and configures ADC input channels based on the provided configuration.
 *
 * This function sets the input channels (positive and/or negative) for the ADC,
 * depending on whether configurable inputs are enabled via Kconfig.
 *
 * - If `CONFIG_ADC_CONFIGURABLE_INPUTS` is defined, it configures the individual
 *   positive and negative inputs.
 * - Otherwise, it selects the channel based on `channel_id` and `differential` flags.
 *
 * @param[in] hal_adc Pointer to the ADC HAL instance.
...
static int adc_get_sample_length(uint16_t acq_time, uint32_t adc_clk, uint8_t prescalar)
{
	...
	case ADC_ACQ_TIME_MICROSECONDS:
		sample_length = ADC_CALC_SAMPLEN_NS(((ADC_ACQ_TIME_VALUE(acq_time)) * 1000),
						    adc_clk, prescalar);
		break;
	case ADC_ACQ_TIME_NANOSECONDS:
		sample_length =
			ADC_CALC_SAMPLEN_NS(ADC_ACQ_TIME_VALUE(acq_time), adc_clk, prescalar);
		break;
	default:
 ...
 * settings for a given ADC channel. It retrieves the hardware-specific
 * ADC handle and uses HAL functions to apply the settings.
 *
				       ...
				       struct adc_channel_cfg *channel_config)
{
	...
	do {
		...
		/* Set Reference */
		adc_set_reference(ADC_REGS, channel_config->reference);
		if (ret < 0) {
			LOG_ERR("Invalid refernce");
			break;
		}
...
static void adc_start_channel(const struct device *dev)
{
	 ...
	 * so error handling is not required at this point.
	 */

	/* Set config for the specific channel in the sequence and trigger the conversion */
...
static void adc_mchp_isr(const struct device *dev)
{
	...
	*dev_data->buffer++ = result;
	...
	if (dev_data->channels != 0) {
		...
		/* Don't call sampling done, instead complete all the channels sampling */
				  ...
				  const struct adc_channel_cfg *channel_cfg)
{
	...
	do {
		...
		/* Calculate sample length from acquition time */
		 ...
		 * it can be configured during channel sequencing. But validate the
		 * channel configuration paramaeters.
		 */

		ret = adc_validate_ch_cfg_params(channel_cfg->gain, channel_cfg->reference,
...
static int adc_mchp_init(const struct device *dev)
{
	...
	clock_control_on(dev_cfg->adc_clock.clock_dev,
	...
	clock_control_get_rate(((const adc_mchp_dev_config_t *)(dev->config))->adc_clock.clock_dev,
	...
	/* IRQ connect and set factory calibration vlaue */
 ...
 *
 * @param hal_adc Pointer to the HAL ADC structure (unused in this function but typically needed for
 * context).
...
{
	const volatile fuses_sw0_fuses_registers_t *hal_fuse =
		(const volatile fuses_sw0_fuses_registers_t *)SW0_ADDR;

	uint32_t sw0_word = (hal_fuse->FUSES_SW0_WORD_0);
...
#define ADC_MCHP_DATA_DEFN(n)                                                                      \
	...
	static adc_mchp_dev_data_t adc_sam_data_##n = {                                            \
 ...
 *
 * Sets the configuration function, pin control, HAL register mappings,
 * and clock definitions for the ADC instance.
 */
#define ADC_MCHP_CONFIG_DEFN(n)                                                                    \
	static void adc_mchp_config_##n(const struct device *dev);                                 \
	static adc_mchp_dev_config_t adc_sam_cfg_##n = {                                           \
...
#define ADC_MCHP_DEVICE(n)                                                                         \
	...
	DEVICE_DT_INST_DEFINE(n, adc_mchp_init, NULL, &adc_sam_data_##n, &adc_sam_cfg_##n,         \
