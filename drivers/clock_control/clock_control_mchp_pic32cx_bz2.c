...
/**
 * @file clock_mchp_pic32cx_bz2.c
 * @brief Clock control driver for pic32cx_bz2 family devices.
 */
 ...
 *****************************************************************************/
#define DT_DRV_COMPAT microchip_pic32cx_bz2_clock
...
#define CLOCK_MCHP_GCLKPERIPH_ID_MAX (28)

#define CLOCK_MCHP_PBCLK_OFFSET_MAX      (0x150)
#define CLOCK_MCHP_REFCLK_OFFSET_MAX     (0x110)
#define CLOCK_MCHP_GCLKPERIPH_OFFSET_MAX (0x80)
...
static uint32_t clock_get_rate_spll1(cru_registers_t *cru_regs)
{
	...
	if ((cru_regs->CRU_CLKSTAT & CRU_CLKSTAT_SPLL1RDY_Msk) == 0) {
		...
		/* SPLL1 Post Divide Value (0 for 1, 1 for 1.5, 2-255) */
		if (spll1_div == 1) {
			src_freq *= 2;
			spll1_div *= 3;
		} else if (spll1_div == 0) {
...
#define BTZB_PLL_NOT_LOCKED                                                                        \
	...
	if ((clk_ready_tries >= CLK_READY_RETRIES) && BTZB_XTAL_NOT_READY) {
		BTZBSYS_REGS->BTZBSYS_SUBSYS_CNTRL_REG1 |=
			(BTZBSYS_SUBSYS_CNTRL_REG1_subsys_bypass_xtal_ready_Msk);
	...
	if ((clk_ready_tries >= CLK_READY_RETRIES) && BTZB_PLL_NOT_LOCKED) {
		BTZBSYS_REGS->BTZBSYS_SUBSYS_CNTRL_REG1 |=
			BTZBSYS_SUBSYS_CNTRL_REG1_subsys_bypass_pll_lock_Msk;
