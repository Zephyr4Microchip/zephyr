...
/* check if subsystem type and id are
valid. */
...
static int clock_off(const struct clock_mchp_config *config, const union clock_mchp_subsys subsys)
{
	...
	case SUBSYS_TYPE_GCLKGEN:
		...
		if (WAIT_FOR(((gclk_regs->GCLK_SYNCBUSY &
			       (BIT(GCLK_SYNCBUSY_GENCTRL_Pos + inst))) == 0),
			     TIMEOUT_GCLKGEN_BUSY_RDY, NULL) == false) {
...
uint32_t find_oschf_frequency_bitpos(uint32_t freq)
{
...
void clock_osc32k_init(const struct device *dev, bool on_demand)
{
	const struct clock_mchp_config *config = dev->config;
	struct clock_mchp_data *data = dev->data;
	osc32kctrl_registers_t *osc32kctrl_regs = config->osc32kctrl_regs;
...
#define CLOCK_MCHP_PROCESS_XOSC32K(node)                                                           \
	{                                                                                          \
		...
		if ((data->gclkgen_src_on_status & BIT(CLOCK_MCHP_GCLK_SRC_XOSC32K)) == 0) {       \
			...
			/* Important: Initializing it with 1, along with clock enabled, can lead   \
			 * to indefinite wait for the clock to be on, if there is no peripheral    \
			 * request for the clock in the sequence of clock Initialization. If       \
			 * required, better to turn on the clock using API, instead of enabling    \
			 * both (on_demand_en & enable) during startup.                            \
			 */                                                                        \
...
#define CLOCK_MCHP_ITERATE_GCLKGEN(child)                                                          \
	do {                                                                                       \
		...
		uint32_t val32 = 0;                                                                \
		if ((data->gclkgen_src_on_status & BIT(DT_ENUM_IDX(child, gclkgen_src))) != 0) {   \
			break;                                                                     \
		}                                                                                  \
