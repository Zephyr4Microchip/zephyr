			...
			bool on)
{
	...
	uint8_t reg8_val = 0, inst = subsys.bits.inst;

	switch (subsys.bits.type) {
	case SUBSYS_TYPE_XOSC:
		reg16 = &oscctrl_regs->OSCCTRL_XOSCCTRL;
		reg16_val = OSCCTRL_XOSCCTRL_ENABLE_Msk;
		break;
	case SUBSYS_TYPE_OSC48M:
		reg8 = &oscctrl_regs->OSCCTRL_OSC48MCTRL;
		reg8_val = OSCCTRL_OSC48MCTRL_ENABLE_Msk;
		break;
	case SUBSYS_TYPE_FDPLL:
		reg8 = &oscctrl_regs->OSCCTRL_DPLLCTRLA;
	...
	case SUBSYS_TYPE_XOSC32K:
		...
		} else if (((*reg16 & (reg16_val ^ (OSC32KCTRL_XOSC32K_EN1K_Msk |
		...
		break;
	case SUBSYS_TYPE_OSC32K:
		reg32 = &osc32kctrl_regs->OSC32KCTRL_OSC32K;
		reg32_val = (inst == INST_OSC32K_OSC1K) ? OSC32KCTRL_OSC32K_EN1K_Msk
							: OSC32KCTRL_OSC32K_EN32K_Msk;
		if (on == true) {
			/* turn on OSC32K if any of EN1K or EN32K is to be on */
			osc32kctrl_regs->OSC32KCTRL_OSC32K |= OSC32KCTRL_OSC32K_ENABLE_Msk;
		} else if (((*reg32 & (reg32_val ^ (OSC32KCTRL_OSC32K_EN1K_Msk |
						    OSC32KCTRL_OSC32K_EN32K_Msk))) == 0)) {
	...
	if (ret_val == CLOCK_SUCCESS) {
		if ((reg32 != NULL) && (on == true)) {
			*reg32 |= reg32_val;
		} else if ((reg32 != NULL) && (on != true)) {
			*reg32 &= ~reg32_val;
		} else if ((reg16 != NULL) && (on == true)) {
			*reg16 |= reg16_val;
		} else if ((reg16 != NULL) && (on != true)) {
			*reg16 &= ~reg16_val;
		} else if ((reg8 != NULL) && (on == true)) {
			*reg8 |= reg8_val;
		} else if ((reg8 != NULL) && (on != true)) {
			*reg8 &= ~reg8_val;
		}
...
static int clock_get_rate_fdpll(const struct device *dev, uint32_t *freq)
{
	...
	clock_mchp_data_t *data = dev->data;
	int ret_val;
	uint32_t src_freq, xosc_div, mult_int, mult_frac, frac_mult_max;
	...
	do {
		...
		case SUBSYS_TYPE_XOSC:
			...
			if ((oscctrl_regs->OSCCTRL_XOSCCTRL & OSCCTRL_XOSCCTRL_ENABLE_Msk) != 0) {
				...
				ret_status = ((oscctrl_regs->OSCCTRL_STATUS &
					       OSCCTRL_STATUS_XOSCRDY_Msk) == 0)
		...
		case SUBSYS_TYPE_OSC48M:
			...
			if ((oscctrl_regs->OSCCTRL_OSC48MCTRL & OSCCTRL_OSC48MCTRL_ENABLE_Msk) !=
			    0) {
				/* Check if ready bit is set */
				ret_status = ((oscctrl_regs->OSCCTRL_STATUS &
					       OSCCTRL_STATUS_OSC48MRDY_Msk) == 0)
						     ? CLOCK_CONTROL_STATUS_STARTING
						     : CLOCK_CONTROL_STATUS_ON;
			} else {
				ret_status = CLOCK_CONTROL_STATUS_OFF;
	...
	do {
		...
		case SUBSYS_TYPE_RTC:
			...
			case OSC32KCTRL_RTCCTRL_RTCSEL_OSC1K_Val:
				mask = OSC32KCTRL_OSC32K_ENABLE_Msk | OSC32KCTRL_OSC32K_EN1K_Msk;
				*freq = ((osc32kctrl_regs->OSC32KCTRL_OSC32K & mask) == mask)
						? FREQ_1KHZ
						: 0;
				break;
		...
		case SUBSYS_TYPE_OSC32K:
			...
			} else if (inst == INST_OSC32K_OSC32K) {
				*freq = FREQ_32KHZ;
			}
			break;
	...
	do {
		...
		if (src > CLOCK_MCHP_FDPLL_SRC_GCLK8) {
			val32 |= OSCCTRL_DPLLCTRLB_REFCLK(
				(src == CLOCK_MCHP_FDPLL_SRC_XOSC32K)
