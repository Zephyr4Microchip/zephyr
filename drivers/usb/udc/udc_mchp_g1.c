...
static inline void udc_load_padcal(const struct device *dev)
{
	...
	for (size_t i = 0U; i < cfg->calib_len; i += CALIB_TUPLE_WIDTH) {
		...
		addr = cfg->nvm_reg + ((bit >> OTP_WORD_SHIFT) << OTP_BYTE_SHIFT);
...
static int udc_mchp_enable_clocks(const struct device *dev)
{
	...
	ret = clock_control_configure(config->clock.clock_dev, config->clock.gclk_subsys,
				      &gclk_cfg);
	if (ret < 0) {
...
{
	__ASSERT_NO_MSG(((max_packet_size & (max_packet_size - 1U)) == 0U &&
...
static int udc_handle_evt_finished(const struct device *dev, struct udc_ep_config *const ep_cfg)
{
	struct net_buf *buf;
...
static void udc_handle_xfer_finished(const struct device *const dev)
{
	...
	while (eps) {
		...
		err = udc_handle_evt_finished(dev, ep_cfg);
...
static void udc_mchp_isr_handler(const struct device *dev)
{
	...
	if ((intflag & USB_DEVICE_INTFLAG_SUSPEND_Msk) && !udc_is_suspended(dev)) {
		udc_set_suspended(dev, true);
		udc_submit_event(dev, UDC_EVT_SUSPEND, 0);
	}

	if ((intflag & USB_DEVICE_INTFLAG_EORSM_Msk) && udc_is_suspended(dev)) {
...
static int udc_mchp_ep_clear_halt(const struct device *dev, struct udc_ep_config *const ep_cfg)
{
	...
	if ((udc_ep_is_busy(ep_cfg) == false) && (udc_buf_peek(ep_cfg) != NULL)) {
		atomic_set_bit(&priv->xfer_new, udc_ep_to_bnum(ep_cfg->addr));
		k_event_post(&priv->events, BIT(MCHP_EVT_XFER_NEW));
	}
 ...
 */
static int udc_mchp_register_eps(const struct device *dev, struct udc_ep_config *ep_cfg,
...
static int udc_mchp_driver_preinit(const struct device *dev)
{
	...
	err = udc_mchp_register_eps(dev, config->ep_cfg_out, config->num_of_eps, USB_EP_DIR_OUT,
				    mps);
	if (err) {
		return err;
	}

	err = udc_mchp_register_eps(dev, config->ep_cfg_in, config->num_of_eps, USB_EP_DIR_IN, mps);
	if (err) {
		return err;
	}
...
#define UDC_MCHP_CONFIG_DEFINE(n)                                                                  \
	...
	static const struct udc_mchp_config udc_mchp_config_##n = {                                \
		...
		.clock.gclk_src = (enum clock_mchp_gclkgen)DT_ENUM_IDX(                            \
			UDC_MCHP_GCLKPERIPH_NODE(n), gclkperiph_src),                              \
