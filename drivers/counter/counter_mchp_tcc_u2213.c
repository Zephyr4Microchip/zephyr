...
LOG_MODULE_REGISTER(counter_mchp_tcc_u2213, CONFIG_COUNTER_LOG_LEVEL);

/**
 * @section Macros
 * User-specific macros for the Microchip counter driver.
 */
#define DT_DRV_COMPAT microchip_counter_tcc_u2213
 ...
 */
#define COUNTER_MCHP_GET_TCC_PRESCALE_INDEX(prescaler)                                             \
	({                                                                                         \
...
/**
 * @brief Number of supported TCC channels.
 */
 ...
 */
static int32_t tcc_counter_init(tcc_registers_t *const pRegs, uint32_t prescaler,
				const uint32_t max_bit_width)
{
	...
	/* Reset TCC */
	pRegs->TCC_CTRLA |= TCC_CTRLA_SWRST_Msk;
	tcc_counter_wait_sync(&pRegs->TCC_SYNCBUSY, TCC_SYNCBUSY_SWRST_Msk);

	/* Configure counter mode & prescaler */
	pRegs->TCC_CTRLA = TCC_CTRLA_CPTEN0(0u) | TCC_CTRLA_CPTEN1(0u) | TCC_CTRLA_CPTEN2(0u) |
			   TCC_CTRLA_CPTEN3(0u) | TCC_CTRLA_CPTEN4(0u) | TCC_CTRLA_CPTEN5(0u) |
			   TCC_CTRLA_MSYNC(0u) |
			   TCC_CTRLA_PRESCALER(COUNTER_MCHP_GET_TCC_PRESCALE_INDEX(prescaler)) |
			   TCC_CTRLA_PRESCSYNC(TCC_GCLK_RESET_ON_GENERIC_CLOCK) |
			   TCC_CTRLA_RUNSTDBY(0u) | TCC_CTRLA_RESOLUTION(0u);

	/* Configure waveform generation mode */
	pRegs->TCC_WAVE = TCC_WAVE_WAVEGEN_NFRQ;

	/* Configure timer one shot mode & direction */
	pRegs->TCC_CTRLBSET = TCC_CTRLBCLR_ONESHOT(0u) | TCC_CTRLBCLR_DIR(0u);

	/* Configure drive control register */
	pRegs->TCC_DRVCTRL = TCC_DRVCTRL_INVEN(0u);

	/* Set the period register to maximum value */
	max_counter_value = (uint32_t)((uint64_t)1u << max_bit_width) - 1u;

	pRegs->TCC_PER = max_counter_value;

	for (uint8_t index = 0u; index < 6u; index++) {
		pRegs->TCC_CC[index] = max_counter_value;
	}

	/* Clear all interrupt flags */
	pRegs->TCC_INTFLAG = TCC_INTFLAG_Msk;

	/* Event control register */
	pRegs->TCC_EVCTRL = TCC_EVCTRL_EVACT0(0u) | TCC_EVCTRL_EVACT1(0u) | TCC_EVCTRL_TCINV(0u) |
	...
	tcc_counter_wait_sync(&pRegs->TCC_SYNCBUSY, ALL_TCC_SYNC_BITS);
 ...
 */
static int32_t tcc_counter_start(tcc_registers_t *const pRegs)
{
	int32_t return_value = COUNTER_RET_PASSED;

	/** Set enable bit */
	pRegs->TCC_CTRLA |= TCC_CTRLA_ENABLE_Msk;

	/** Wait for synchronization */
	tcc_counter_wait_sync(&pRegs->TCC_SYNCBUSY, TCC_SYNCBUSY_ENABLE_Msk);

	/* Write command to force COUNT register read synchronization */
	pRegs->TCC_CTRLBSET |= TCC_CTRLBSET_CMD_RETRIGGER;

	/** Wait for synchronization */
	tcc_counter_wait_sync(&pRegs->TCC_SYNCBUSY, TCC_SYNCBUSY_CTRLB_Msk);

	/* Wait for CMD to become zero */
	while ((pRegs->TCC_CTRLBSET & TCC_CTRLBSET_CMD_Msk) != 0U) {
	}
 ...
 */
static int32_t tcc_counter_stop(tcc_registers_t *const pRegs)
{
	int32_t return_value = COUNTER_RET_PASSED;

	/* Write command to force COUNT register read synchronization */
	pRegs->TCC_CTRLBSET |= TCC_CTRLBSET_CMD_STOP;
 ...
 */
static int32_t tcc_counter_retrigger(tcc_registers_t *const pRegs)
{
	int32_t return_value = COUNTER_RET_PASSED;
 ...
 * @retval 0 Success.
 */
static int32_t tcc_counter_get_count(tcc_registers_t *const pRegs, uint32_t *const counter_value)
{
	int32_t ret_status = 0;

	/* Write command to force COUNT register read synchronization */
	pRegs->TCC_CTRLBSET |= TCC_CTRLBSET_CMD_READSYNC;
 ...
 */
static int32_t tcc_counter_set_period(tcc_registers_t *const pRegs, const uint32_t period)
{
	int32_t return_value = COUNTER_RET_PASSED;

	/** Set the period value */
	pRegs->TCC_PER = period;

	/** Wait for synchronization */
	tcc_counter_wait_sync(&pRegs->TCC_SYNCBUSY, TCC_SYNCBUSY_PER_Msk);
	return return_value;
}
 ...
 */
static uint32_t tcc_counter_get_period(tcc_registers_t *const pRegs)
{
	return pRegs->TCC_PER;
}
 ...
 */
static int32_t tcc_counter_set_compare(tcc_registers_t *const pRegs, const uint32_t chan_id,
				       const uint32_t compare_value)
{
	int32_t return_value = COUNTER_RET_PASSED;

	/** Set the period value */
	pRegs->TCC_CC[chan_id] = compare_value;
 ...
 */
static int32_t tcc_counter_get_pending_irqs(tcc_registers_t *const pRegs)
{
	/** Check interrupt flags */
	return pRegs->TCC_INTFLAG;
}
 ...
 */
static int32_t tcc_counter_alarm_irq_enable(tcc_registers_t *const pRegs, const uint32_t channel_id)
{
	if (channel_id >= TCC_CHANNEL_COUNT) {
		return COUNTER_RET_FAILED;
	}
	pRegs->TCC_INTENSET = TCC_INTFLAG_MC0_Msk << channel_id;
 ...
 * @retval COUNTER_RET_FAILED Invalid channel.
 */
static int32_t tcc_counter_alarm_irq_disable(tcc_registers_t *const pRegs,
					     const uint32_t channel_id)
{
	if (channel_id >= TCC_CHANNEL_COUNT) {
		return COUNTER_RET_FAILED;
	}
	pRegs->TCC_INTENCLR = TCC_INTFLAG_MC0_Msk << channel_id;
	return COUNTER_RET_PASSED;
}
 ...
 * @retval COUNTER_RET_FAILED Invalid channel.
 */
static int32_t tcc_counter_alarm_irq_clear(tcc_registers_t *const pRegs, const uint32_t channel_id)
{
	if (channel_id >= TCC_CHANNEL_COUNT) {
		return COUNTER_RET_FAILED;
	}
	pRegs->TCC_INTFLAG = TCC_INTFLAG_MC0_Msk << channel_id;
	return COUNTER_RET_PASSED;
}
 ...
 * @retval COUNTER_RET_PASSED Success.
 */
static int32_t tcc_counter_top_irq_enable(tcc_registers_t *const pRegs)
{
	int32_t return_value = COUNTER_RET_PASSED;

	/* Enable overflow interrupt */
	pRegs->TCC_INTENSET = TCC_INTFLAG_OVF_Msk;
 ...
 * @retval COUNTER_RET_PASSED Success.
 */
static int32_t tcc_counter_top_irq_disable(tcc_registers_t *const pRegs)
{
	int32_t return_value = COUNTER_RET_PASSED;

	/* Disable overflow interrupt */
	pRegs->TCC_INTENCLR = TCC_INTFLAG_OVF_Msk;
 ...
 * @retval COUNTER_RET_PASSED Success.
 */
static int32_t tcc_counter_top_irq_clear(tcc_registers_t *const pRegs)
{
	/* Clear overflow interrupt */
	pRegs->TCC_INTFLAG = TCC_INTFLAG_OVF_Msk;
...
/**
 * @brief Configure the IRQ handler for the Microchip TCC U2213 counter.
 *
 * This macro sets up the IRQ handler for the specified instance of the
 * Microchip TCC U2213 counter. It connects the appropriate IRQs based on
 ...
 *		 connects them sequentially. It supports up to 6 IRQs for the TCC U2213 counter.
 */
/* clang-format off */
...
/**
 * @brief Macro to determine the number of IRQs for Microchip TCC U2213.
 *
 * This macro calculates the number of IRQs for the given instance of the Microchip TCC U2213
...
/**
 * @brief Initialize Microchip TCC U2213 counter devices.
 *
 * This block checks for the presence of TCC U2213 counter devices in the device tree
