 ...
 *
 * This section includes necessary header files for the implementation.
 */
#include <soc.h>
...
LOG_MODULE_REGISTER(counter_mchp_tc_u2249, CONFIG_COUNTER_LOG_LEVEL);
...
/** @brief Device tree compatible string for the driver. */
#define DT_DRV_COMPAT microchip_counter_tc_u2249

/** @brief All timer/counter synchronization bits set. */
#define ALL_TC_SYNC_BITS ((uint32_t)UINT32_MAX)

/** @brief Return value for failed operation. */
#define COUNTER_RET_FAILED (-1)

/** @brief Return value for successful operation. */
...
/**
 * @brief Enumeration for counter bit modes.
 */
typedef enum {
...
/**
 * @brief Enumeration for counter reset modes.
 */
typedef enum {
...
/**
 * @brief Enumeration for event control modes.
 */
typedef enum {
			       ...
			       const uint32_t max_bit_width)
{
	...
	uint32_t ctrla_reg_value =
		TC_CTRLA_CAPTEN0(0u) | TC_CTRLA_CAPTEN1(0u) | TC_CTRLA_COPEN0(0u) |
		TC_CTRLA_COPEN1(0u) |
		TC_CTRLA_PRESCALER(COUNTER_MCHP_GET_TC_PRESCALE_INDEX(prescaler)) |
		TC_CTRLA_PRESCSYNC(TC_GCLK_RESET_ON_GENERIC_CLOCK) | TC_CTRLA_ONDEMAND(0u) |
		TC_CTRLA_RUNSTDBY(0u);
	...
	case COUNTER_BIT_MODE_32: {
		tc_count32_registers_t *const pRegs = &(((tc_registers_t *)tc_regs)->COUNT32);

		if (TC_STATUS_Msk != (pRegs->TC_STATUS & TC_STATUS_Msk)) {
			pRegs->TC_CTRLA &= ~TC_CTRLA_ENABLE_Msk;
			tc_counter_wait_sync(&pRegs->TC_SYNCBUSY, TC_SYNCBUSY_ENABLE_Msk);
			pRegs->TC_CTRLA |= TC_CTRLA_SWRST_Msk;
			tc_counter_wait_sync(&pRegs->TC_SYNCBUSY, TC_SYNCBUSY_SWRST_Msk);

			pRegs->TC_CTRLA = ctrla_reg_value | TC_CTRLA_MODE(2u);
			pRegs->TC_WAVE = TC_WAVE_WAVEGEN_MFRQ;
			pRegs->TC_CTRLBSET = TC_CTRLBCLR_ONESHOT(0u) | TC_CTRLBCLR_DIR(0u);
			pRegs->TC_DRVCTRL = TC_DRVCTRL_INVEN(0u);
			pRegs->TC_CC[0u] = UINT32_MAX;
			pRegs->TC_CC[1u] = UINT32_MAX;
			pRegs->TC_INTFLAG = TC_INTFLAG_Msk;
			pRegs->TC_EVCTRL = evctrl_reg_value;
			tc_counter_wait_sync(&pRegs->TC_SYNCBUSY, ALL_TC_SYNC_BITS);
		} else {
	...
	case COUNTER_BIT_MODE_16: {
		tc_count16_registers_t *pRegs = &(((tc_registers_t *)tc_regs)->COUNT16);
		...
		if (TC_STATUS_Msk != (pRegs->TC_STATUS & TC_STATUS_Msk)) {
			...
			pRegs->TC_CTRLA = ctrla_reg_value;
			pRegs->TC_WAVE = TC_WAVE_WAVEGEN_MFRQ;
			pRegs->TC_CTRLBSET = TC_CTRLBCLR_ONESHOT(0u) | TC_CTRLBCLR_DIR(0u);
			pRegs->TC_DRVCTRL = TC_DRVCTRL_INVEN(0u);
			pRegs->TC_CC[0u] = UINT16_MAX;
			pRegs->TC_CC[1u] = UINT16_MAX;
			pRegs->TC_INTFLAG = TC_INTFLAG_Msk;
			pRegs->TC_EVCTRL = evctrl_reg_value;
			tc_counter_wait_sync(&pRegs->TC_SYNCBUSY, ALL_TC_SYNC_BITS);
		} else {
	...
	case COUNTER_BIT_MODE_8: {
		tc_count8_registers_t *pRegs = &(((tc_registers_t *)tc_regs)->COUNT8);
		...
		if (TC_STATUS_Msk != (pRegs->TC_STATUS & TC_STATUS_Msk)) {
			...
			pRegs->TC_CTRLA = ctrla_reg_value | TC_CTRLA_MODE(1u);
			pRegs->TC_WAVE = TC_WAVE_WAVEGEN_NFRQ;
			pRegs->TC_CTRLBSET = TC_CTRLBCLR_ONESHOT(0u) | TC_CTRLBCLR_DIR(0u);
			pRegs->TC_DRVCTRL = TC_DRVCTRL_INVEN(0u);
			pRegs->TC_CC[0u] = UINT8_MAX;
			pRegs->TC_CC[1u] = UINT8_MAX;
			pRegs->TC_PER = UINT8_MAX;
			pRegs->TC_INTFLAG = TC_INTFLAG_Msk;
			pRegs->TC_EVCTRL = evctrl_reg_value;
			tc_counter_wait_sync(&pRegs->TC_SYNCBUSY, ALL_TC_SYNC_BITS);
		} else {
...
static int32_t tc_counter_start(const void *tc_regs, const uint32_t max_bit_width)
{
	...
	switch (max_bit_width) {
	case COUNTER_BIT_MODE_32: {
		tc_count32_registers_t *pRegs = &(((tc_registers_t *)tc_regs)->COUNT32);

		pRegs->TC_CTRLA |= TC_CTRLA_ENABLE_Msk;
		tc_counter_wait_sync(&pRegs->TC_SYNCBUSY, TC_SYNCBUSY_ENABLE_Msk);

		pRegs->TC_CTRLBSET |= TC_CTRLBSET_CMD_RETRIGGER;
		tc_counter_wait_sync(&pRegs->TC_SYNCBUSY, TC_SYNCBUSY_CTRLB_Msk);
		while ((pRegs->TC_CTRLBSET & TC_CTRLBSET_CMD_Msk) != 0U) {
		}
		break;
	}
	case COUNTER_BIT_MODE_16: {
...
static int32_t tc_counter_stop(const void *tc_regs, const uint32_t max_bit_width)
{
	...
	case COUNTER_BIT_MODE_32: {
		...
		pRegs->TC_CTRLBSET |= TC_CTRLBSET_CMD_STOP;
		tc_counter_wait_sync(&pRegs->TC_SYNCBUSY, TC_SYNCBUSY_CTRLB_Msk);
		while ((pRegs->TC_CTRLBSET & TC_CTRLBSET_CMD_Msk) != 0U) {
		}
		break;
					   ...
					   const uint32_t max_bit_width)
{
	...
	case COUNTER_BIT_MODE_32: {
		...
		pRegs->TC_CTRLBSET |= TC_CTRLBSET_CMD_READSYNC;
		tc_counter_wait_sync(&pRegs->TC_SYNCBUSY, TC_SYNCBUSY_CTRLB_Msk);
		while ((pRegs->TC_CTRLBSET & TC_CTRLBSET_CMD_Msk) != 0U) {
		}
		*counter_value = pRegs->TC_COUNT;
		break;
	}
	case COUNTER_BIT_MODE_16: {
		tc_count16_registers_t *pRegs = &(((tc_registers_t *)tc_regs)->COUNT16);
				     ...
				     const uint32_t max_bit_width)
{
	...
	case COUNTER_BIT_MODE_32: {
		...
		pRegs->TC_CC[0u] = period;
		tc_counter_wait_sync(&pRegs->TC_SYNCBUSY, TC_SYNCBUSY_CC0_Msk);
		break;
	}
	case COUNTER_BIT_MODE_16: {
		tc_count16_registers_t *pRegs = &(((tc_registers_t *)tc_regs)->COUNT16);
				      ...
				      const uint32_t compare_value, const uint32_t max_bit_width)
{
	...
	case COUNTER_BIT_MODE_32: {
		...
		pRegs->TC_CC[1u] = compare_value;
		tc_counter_wait_sync(&pRegs->TC_SYNCBUSY, TC_SYNCBUSY_CC1_Msk);
		break;
	}
	case COUNTER_BIT_MODE_16: {
		(void)chan_id;
	...
	case COUNTER_BIT_MODE_8: {
		...
		pRegs->TC_CC[chan_id] = compare_value;
		if (chan_id == 0u) {
			tc_counter_wait_sync(&pRegs->TC_SYNCBUSY, TC_SYNCBUSY_CC0_Msk);
		} else {
			tc_counter_wait_sync(&pRegs->TC_SYNCBUSY, TC_SYNCBUSY_CC1_Msk);
					   ...
					   const uint32_t max_bit_width)
{
	...
	case COUNTER_BIT_MODE_32: {
		...
		if (channel_id == 0u) {
			pRegs->TC_INTENSET = TC_INTFLAG_MC1_Msk;
		} else {
			return_value = COUNTER_RET_FAILED;
		}
		break;
	...
	case COUNTER_BIT_MODE_8: {
		...
		if (channel_id == 0u) {
			pRegs->TC_INTENSET = TC_INTFLAG_MC0_Msk;
					    ...
					    const uint32_t max_bit_width)
{
	...
	case COUNTER_BIT_MODE_32: {
		...
		if (channel_id == 0u) {
			pRegs->TC_INTENCLR = TC_INTFLAG_MC1_Msk;
		} else {
			return_value = COUNTER_RET_FAILED;
		}
		break;
	...
	case COUNTER_BIT_MODE_8: {
		...
		if (channel_id == 0u) {
			pRegs->TC_INTENCLR = TC_INTFLAG_MC0_Msk;
		} else if (channel_id == 1u) {
			pRegs->TC_INTENCLR = TC_INTFLAG_MC1_Msk;
		} else {
			return_value = COUNTER_RET_FAILED;
					  ...
					  const uint32_t max_bit_width)
{
	...
	case COUNTER_BIT_MODE_32: {
		...
		if (channel_id == 0u) {
			pRegs->TC_INTFLAG = TC_INTFLAG_MC1_Msk;
		} else {
			return_value = COUNTER_RET_FAILED;
		}
		break;
	...
	case COUNTER_BIT_MODE_8: {
		...
		if (channel_id == 0u) {
			pRegs->TC_INTFLAG = TC_INTFLAG_MC0_Msk;
		} else if (channel_id == 1u) {
			pRegs->TC_INTFLAG = TC_INTFLAG_MC1_Msk;
		} else {
			return_value = COUNTER_RET_FAILED;
...
static int32_t tc_counter_top_irq_enable(const void *tc_regs, const uint32_t max_bit_width)
{
	...
	case COUNTER_BIT_MODE_8: {
		...
		pRegs->TC_INTENSET = TC_INTFLAG_OVF_Msk;
		break;
	}
	default:
		LOG_ERR("%s : Unsupported Counter mode %d", __func__, max_bit_width);
...
static int32_t tc_counter_top_irq_disable(const void *tc_regs, const uint32_t max_bit_width)
{
	...
	case COUNTER_BIT_MODE_8: {
		...
		pRegs->TC_INTENCLR = TC_INTFLAG_OVF_Msk;
		break;
	}
	default:
		LOG_ERR("%s : Unsupported Counter mode %d", __func__, max_bit_width);
...
static int32_t tc_counter_top_irq_clear(const void *tc_regs, const uint32_t max_bit_width)
{
	...
	case COUNTER_BIT_MODE_8: {
		...
		pRegs->TC_INTFLAG = TC_INTFLAG_OVF_Msk;
		break;
	}
	default:
		LOG_ERR("%s : Unsupported Counter mode %d", __func__, max_bit_width);
 ...
 * counter driver API.
 */
static const struct counter_driver_api counter_mchp_driver_api = {
...
/**
 * @brief Macro to determine the number of compare/capture channels for Microchip TC U2249.
 *
...
/**
 * @brief Determine the maximum bit width for the counter.
 *
 * This macro calculates the maximum bit width for a counter based on the presence
 * of a "client" property in the device tree and the requested bit width. If the
 * "client" property is not present and 32-bit mode is requested, the bit width
 * is limited to 16 bits. Otherwise, it uses the value specified in the device tree.
 *
 * @param n	 Counter instance number.
 *
 * @note This macro relies on device tree properties to determine the configuration.
 *		 - `DT_NODE_HAS_PROP`: Checks if a property exists in the device tree.
 *		 - `DT_NODELABEL`: Retrieves the node label for the counter instance.
 *		 - `DT_INST_PROP`: Retrieves the property value for the instance.
 *		 - `COND_CODE_1`: Conditional evaluation based on a boolean expression.
 */
/* clang-format off */
#define COUNTER_MCHP_MAX_BIT_WIDTH(n)	\
	COND_CODE_1(	\
		DT_NODE_HAS_PROP(DT_DRV_INST(n), client),	\
		(DT_INST_PROP(n, max_bit_width)),	\
		(	\
			COND_CODE_1(	\
				DT_ENUM_HAS_VALUE(	\
					DT_DRV_INST(n), max_bit_width, 32),	\
				(16),	\
				(	\
					COND_CODE_1(	\
						DT_ENUM_HAS_VALUE(	\
...
#define COUNTER_MCHP_CLOCK_ASSIGN(n)  \
   ...
   \
	COND_CODE_1(   \
		DT_NODE_EXISTS(DT_INST_CLOCKS_CTLR_BY_NAME(n, gclk)),	\
...
/**
 * @brief Enable all necessary clocks for the Counter peripheral.
 *
 * This macro enables the host core synchronous clock, client core
 * synchronous clock (if applicable), and the peripheral asynchronous
 * clock, based on the device tree configuration for the given instance.
 *
 * @param n	   Counter instance number (used for device tree lookups).
 * @param dev  Device structure pointer for the Counter peripheral.
 */
/* clang-format off */
#define COUNTER_MCHP_ENABLE_CLOCK(n, dev)	\
do {	\
	ret_status = COUNTER_RET_PASSED;	\
	const counter_mchp_dev_cfg_t *cfg =	\
			(const counter_mchp_dev_cfg_t *)(dev->config);	\
	counter_mchp_clock_t *const clk = \
	(counter_mchp_clock_t *const)&cfg->counter_clock;	\
	\
	/* Enable host synchronous core clock */	\
	ret_status = clock_control_on(clk->clock_dev,	\
			clk->host_core_sync_clk);	\
	...
	COND_CODE_1(	\
		...
		if (ret_status != COUNTER_RET_FAILED) {	\
			if (cfg->max_bit_width == 32) {	\
				ret_status = clock_control_on(clk->clock_dev,	\
							clk->client_core_sync_clk);	\
				if ((ret_status < 0) &&	\
					(ret_status != -EALREADY)) {	\
					LOG_ERR("%s : Unable to initialize client clock",	\
								__func__);	\
					ret_status = COUNTER_RET_FAILED;	\
				}	\
			}	\
	...
	if (ret_status != COUNTER_RET_FAILED) {	\
		...
		ret_status = clock_control_on(clk->clock_dev,	\
				clk->periph_async_clk);	\
		if ((ret_status < 0) && (ret_status != -EALREADY)) {	\
			LOG_ERR("%s : Unable to initialize peripheral clock",	\
					__func__);	\
			ret_status = COUNTER_RET_FAILED;	\
		}	\
	}	\
...
#define COUNTER_MCHP_DEVICE_INIT_FUNC(n)	\
	...
	\
	static int32_t counter_mchp_##n##_init(const struct device *dev)	\
	{	\
		int32_t ret_status = COUNTER_RET_PASSED;	\
		\
		/**< Enable the clock for the device instance. */	 \
		COUNTER_MCHP_ENABLE_CLOCK(n, dev);	  \
		\
		if (ret_status != COUNTER_RET_FAILED) {	\
			/**< Call the main initialization function for the device. */	 \
			ret_status =  counter_mchp_init(dev);	  \
		} else {	\
			ret_status = COUNTER_RET_FAILED;	\
		}	\
		return ret_status;	\
...
#define COUNTER_MCHP_DEVICE_INIT(n)	   \
	...
	DEVICE_DT_INST_DEFINE(n, counter_mchp_##n##_init, NULL,	   \
				  &counter_mchp_dev_data_##n,	 \
				  &counter_mchp_dev_config_##n, POST_KERNEL,	\
				  CONFIG_COUNTER_INIT_PRIORITY, &counter_mchp_driver_api);
/* clang-format on */

/**
 * @brief Initialize Microchip TC U2249 counter devices.
 *
 * This block checks for the presence of TC U2249 counter devices in the device tree
