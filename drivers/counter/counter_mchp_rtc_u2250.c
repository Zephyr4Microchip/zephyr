...
/* Register the logging module for this driver */
LOG_MODULE_REGISTER(counter_mchp_rtc_u2250, CONFIG_COUNTER_LOG_LEVEL);

/**
 * @brief Macro functions.
 */

/* Device Tree compatible string for Microchip Counter RTC U2250 */
#define DT_DRV_COMPAT microchip_counter_rtc_u2250
...
/* Macro to get prescaler index */
#define COUNTER_MCHP_GET_RTC_PRESCALE_INDEX(prescaler)                                             \
	({                                                                                         \
 ...
 * @param pRegs Pointer to the RTC register structure containing RTC_COUNT.
 */
					  ...
					  const uint32_t max_bit_width)
{
	...
	case COUNTER_BIT_MODE_32: {
		rtc_mode0_registers_t *const pRegs =
			(rtc_mode0_registers_t *const)(&(((rtc_registers_t *)regs)->MODE0));

		if ((pRegs->RTC_CTRLA & RTC_MODE0_CTRLA_ENABLE_Msk) != 0) {
			bool success =
				WAIT_FOR((*counter_value != pRegs->RTC_COUNT),
					 RTC_SYNCHRONIZATION_TIMEOUT_IN_US, k_busy_wait(DELAY_US));
			if (!success) {
	...
	case COUNTER_BIT_MODE_16: {
		rtc_mode1_registers_t *const pRegs =
 ...
 *	- Disables all output events, interrupts, and capture modes.
...
static int32_t rtc_counter_init(const void *regs, uint32_t prescaler, const uint32_t max_bit_width)
{
	...
	case COUNTER_BIT_MODE_32: {
		...
		pRegs->RTC_CTRLA &= ~RTC_MODE0_CTRLA_ENABLE_Msk;
		rtc_counter_wait_sync(&pRegs->RTC_SYNCBUSY, RTC_MODE0_SYNCBUSY_ENABLE_Msk);

		pRegs->RTC_CTRLA = RTC_MODE0_CTRLA_SWRST_Msk;
		rtc_counter_wait_sync(&pRegs->RTC_SYNCBUSY, RTC_MODE0_SYNCBUSY_SWRST_Msk);

		pRegs->RTC_CTRLA =
			RTC_MODE0_CTRLA_MODE(0u) | RTC_MODE0_CTRLA_MATCHCLR(1u) |
			RTC_MODE0_CTRLA_COUNTSYNC(1u) |
			RTC_MODE0_CTRLA_PRESCALER(COUNTER_MCHP_GET_RTC_PRESCALE_INDEX(prescaler));

		pRegs->RTC_COMP[0] = UINT32_MAX;
		pRegs->RTC_COMP[1] = UINT32_MAX;
		pRegs->RTC_INTFLAG = RTC_MODE0_INTFLAG_Msk;
		rtc_counter_wait_sync(&pRegs->RTC_SYNCBUSY, ALL_RTC_SYNC_BITS);
		break;
	}
	case COUNTER_BIT_MODE_16: {
		rtc_mode1_registers_t *const pRegs =
		...
		pRegs->RTC_CTRLA &= ~RTC_MODE1_CTRLA_ENABLE_Msk;
		rtc_counter_wait_sync(&pRegs->RTC_SYNCBUSY, RTC_MODE1_SYNCBUSY_ENABLE_Msk);

		pRegs->RTC_CTRLA = RTC_MODE1_CTRLA_SWRST_Msk;
		rtc_counter_wait_sync(&pRegs->RTC_SYNCBUSY, RTC_MODE1_SYNCBUSY_SWRST_Msk);

		pRegs->RTC_CTRLA =
			RTC_MODE1_CTRLA_MODE(1u) | RTC_MODE1_CTRLA_COUNTSYNC(1u) |
			RTC_MODE1_CTRLA_PRESCALER(COUNTER_MCHP_GET_RTC_PRESCALE_INDEX(prescaler));

		pRegs->RTC_PER = UINT16_MAX;
		pRegs->RTC_COMP[0u] = UINT16_MAX;
		pRegs->RTC_COMP[1u] = UINT16_MAX;
		pRegs->RTC_COMP[2u] = UINT16_MAX;
		pRegs->RTC_COMP[3u] = UINT16_MAX;
		pRegs->RTC_INTFLAG = RTC_MODE1_INTFLAG_Msk;
		rtc_counter_wait_sync(&pRegs->RTC_SYNCBUSY, ALL_RTC_SYNC_BITS);
		break;
	}
	default:
 ...
 * @retval COUNTER_RET_FAILED   Invalid bit width.
 */
static int32_t rtc_counter_stop(const void *regs, const uint32_t max_bit_width)
{
				      ...
				      const uint32_t max_bit_width)
{
	...
	case COUNTER_BIT_MODE_32: {
		...
		if ((pRegs->RTC_CTRLA & RTC_MODE0_CTRLA_COUNTSYNC_Msk) == 0u) {
			pRegs->RTC_CTRLA |= RTC_MODE0_CTRLA_COUNTSYNC_Msk;
			rtc_counter_wait_sync(&pRegs->RTC_SYNCBUSY,
					      RTC_MODE0_SYNCBUSY_COUNTSYNC_Msk);
		}
		rtc_counter_wait_sync(&pRegs->RTC_SYNCBUSY, RTC_MODE0_SYNCBUSY_COUNT_Msk);

		*counter_value = pRegs->RTC_COUNT;
				      ...
				      const uint32_t max_bit_width)
{
	...
	case COUNTER_BIT_MODE_32: {
		rtc_mode0_registers_t *const pRegs =
			(rtc_mode0_registers_t *const)(&(((rtc_registers_t *)regs)->MODE0));
		pRegs->RTC_COMP[0] = period;
		rtc_counter_wait_sync(&pRegs->RTC_SYNCBUSY, RTC_MODE0_SYNCBUSY_COMP0_Msk);
		break;
	}
	case COUNTER_BIT_MODE_16: {
		rtc_mode1_registers_t *const pRegs =
			(rtc_mode1_registers_t *const)(&(((rtc_registers_t *)regs)->MODE1));
		pRegs->RTC_PER = period;
		rtc_counter_wait_sync(&pRegs->RTC_SYNCBUSY, RTC_MODE1_SYNCBUSY_PER_Msk);
		break;
	}
	default:
		LOG_ERR("%s : Unsupported Counter mode %d", __func__, max_bit_width);
				      ...
				      const uint32_t max_bit_width)
{
	...
	case COUNTER_BIT_MODE_32: {
		rtc_mode0_registers_t *const pRegs =
			(rtc_mode0_registers_t *const)(&(((rtc_registers_t *)regs)->MODE0));
		*period = pRegs->RTC_COMP[0];
		break;
	}
	case COUNTER_BIT_MODE_16: {
		rtc_mode1_registers_t *const pRegs =
			(rtc_mode1_registers_t *const)(&(((rtc_registers_t *)regs)->MODE1));
		*period = pRegs->RTC_PER;
		break;
	}
	default:
		LOG_ERR("%s : Unsupported Counter mode %d", __func__, max_bit_width);
				       ...
				       const uint32_t compare_value, const uint32_t max_bit_width)
{
	...
	case COUNTER_BIT_MODE_32: {
		...
		pRegs->RTC_COMP[1u] = compare_value;
		rtc_counter_wait_sync(&pRegs->RTC_SYNCBUSY, ALL_RTC_SYNC_BITS);
		break;
	}
	case COUNTER_BIT_MODE_16: {
		...
		pRegs->RTC_COMP[chan_id] = compare_value;
		rtc_counter_wait_sync(&pRegs->RTC_SYNCBUSY, ALL_RTC_SYNC_BITS);
		break;
	}
	default:
					    ...
					    const uint32_t max_bit_width)
{
	...
	case COUNTER_BIT_MODE_32: {
		...
		if (channel_id == 0u) {
			pRegs->RTC_INTENSET = RTC_MODE0_INTFLAG_CMP1_Msk;
		} else {
	...
	case COUNTER_BIT_MODE_16: {
		...
		pRegs->RTC_INTENSET = RTC_MODE0_INTFLAG_CMP0_Msk << channel_id;
					     ...
					     const uint32_t max_bit_width)
{
	...
	case COUNTER_BIT_MODE_32: {
		...
		if (channel_id == 0u) {
			pRegs->RTC_INTENCLR = RTC_MODE0_INTFLAG_CMP1_Msk;
		} else {
			ret_status = COUNTER_RET_FAILED;
		}
		break;
	...
	case COUNTER_BIT_MODE_16: {
		...
		pRegs->RTC_INTENCLR = 1U << ((uint32_t)RTC_MODE1_INTFLAG_CMP0_Msk + channel_id);
	}
	default:
		LOG_ERR("%s : Unsupported Counter mode %d", __func__, max_bit_width);
		ret_status = COUNTER_RET_FAILED;
					   ...
					   const uint32_t max_bit_width)
{
	...
	case COUNTER_BIT_MODE_32: {
		...
		if (channel_id == 0u) {
			pRegs->RTC_INTFLAG = RTC_MODE0_INTFLAG_CMP1_Msk;
		} else {
			ret_status = COUNTER_RET_FAILED;
		}
		break;
	...
	case COUNTER_BIT_MODE_16: {
		...
		pRegs->RTC_INTFLAG = RTC_MODE0_INTFLAG_CMP0_Msk << channel_id;
		break;
	}
	default:
		LOG_ERR("%s : Unsupported Counter mode %d", __func__, max_bit_width);
...
static int32_t rtc_counter_top_irq_enable(const void *regs, const uint32_t max_bit_width)
{
	...
	case COUNTER_BIT_MODE_32: {
		rtc_mode0_registers_t *const pRegs =
			(rtc_mode0_registers_t *const)(&(((rtc_registers_t *)regs)->MODE0));
		pRegs->RTC_INTENSET = RTC_MODE0_INTFLAG_CMP0_Msk;
		break;
	}
	case COUNTER_BIT_MODE_16: {
		rtc_mode1_registers_t *const pRegs =
			(rtc_mode1_registers_t *const)(&(((rtc_registers_t *)regs)->MODE1));
		pRegs->RTC_INTENSET = RTC_MODE1_INTFLAG_OVF_Msk;
		break;
	}
	default:
		LOG_ERR("%s : Unsupported Counter mode %d", __func__, max_bit_width);
...
static int32_t rtc_counter_top_irq_disable(const void *regs, const uint32_t max_bit_width)
{
	...
	case COUNTER_BIT_MODE_32: {
		...
		pRegs->RTC_INTENCLR = RTC_MODE0_INTFLAG_CMP0_Msk;
		break;
	}
	case COUNTER_BIT_MODE_16: {
		rtc_mode1_registers_t *const pRegs =
		...
		pRegs->RTC_INTENCLR = RTC_MODE1_INTFLAG_OVF_Msk;
		break;
	}
	default:
		LOG_ERR("%s : Unsupported Counter mode %d", __func__, max_bit_width);
...
static int32_t rtc_counter_top_irq_clear(const void *regs, const uint32_t max_bit_width)
{
	...
	case COUNTER_BIT_MODE_32: {
		...
		pRegs->RTC_INTFLAG = RTC_MODE0_INTFLAG_CMP0_Msk;
		break;
	}
	case COUNTER_BIT_MODE_16: {
		rtc_mode1_registers_t *const pRegs =
		...
		pRegs->RTC_INTFLAG = RTC_MODE1_INTFLAG_OVF_Msk;
		break;
	}
	default:
		LOG_ERR("%s : Unsupported Counter mode %d", __func__, max_bit_width);
...
/**
 * @brief Initialize Microchip RTC U2250 counter devices.
 *
 * This block checks for the presence of RTC U2250 counter devices in the device tree
