...
/**
 * @file reset_mchp_rcon_g1.c
 * @brief Microchip RCON G1 Reset Controller Driver for Zephyr
 *
 * This file implements the driver for the Microchip RCON G1 reset controller,
 * providing APIs to assert, deassert, toggle, and query the status of reset lines.
 *
 */
#define DT_DRV_COMPAT microchip_rcon_g1_reset
...
#define MCHP_RST_LINE_MAX 16
#define RESET_REGS(dev)   (((const reset_mchp_config_t *)((dev)->config))->regs)
/**
 * @brief Configuration structure for the Microchip RCON G1 reset controller.
 */
typedef struct reset_mchp_config {
	rcon_registers_t *regs; /* Pointer to the reset controller registers */
} reset_mchp_config_t;
...
{
	int ret = 0;
	uint8_t rcause = RESET_REGS(dev)->RCON_RCON;
...
/**
 * @brief Reset driver API structure for Microchip RCON G1.
 *
...
/**
 * @brief Configuration instance for the Microchip RCON G1 reset controller.
 */
static const reset_mchp_config_t reset_mchp_config = {
	.regs = (rcon_registers_t *)DT_INST_REG_ADDR(0), /* Get the base address from DT */
};

/**
 * @brief Device instance definition for the Microchip RCON G1 reset controller.
 *
 * This macro defines and registers the device instance with the Zephyr device model.
 */
BUILD_ASSERT(DT_NUM_INST_STATUS_OKAY(DT_DRV_COMPAT) <= 1,
	     "Only one Microchip RCON G1 instance is supported.");
