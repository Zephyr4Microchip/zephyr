...
ZTEST(i2c_eeprom, test_i2c_configure)
{
	int err;
...
#if !defined(CONFIG_I2C_CALLBACK)
ZTEST(i2c_eeprom, test_eeprom_int)
{
	int ret;
...
#if defined(CONFIG_I2C_CALLBACK)
ZTEST(i2c_eeprom, test_eeprom_async)
{
	...
	int ret = i2c_transfer_cb(i2c_dev, tx_msg, 2, EEPROM_I2C_ADDR, i2c_eeprom_callback,
				  &async_sem);
	zassert_equal(ret, 0, "EEPROM write failed: %d", ret);
	...
	ret = i2c_transfer_cb(i2c_dev, rx_msg, 2, EEPROM_I2C_ADDR, i2c_eeprom_callback, &async_sem);
	zassert_equal(ret, 0, "EEPROM read failed: %d", ret);
...
ZTEST_SUITE(i2c_eeprom, NULL, i2c_test_setup, NULL, NULL, NULL);
